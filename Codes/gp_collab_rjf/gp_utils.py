...
class KernelConfig:
    ...
    def default_bounds(self) -> list[tuple[float, float]]:
        ...
        # disjoint (short metric in [exp(-6), exp(1)] = [0.0025, 2.7],
        # long metric in [exp(1), exp(8)] = [2.7, 2980]). This avoids the
        ...
        bounds: dict[str, tuple[float, float]] = {
            ...
            # Spec floor at log(0.005); near-simultaneous spectra are
            # marginally inconsistent at this level. Without a floor the
            # optimizer drives sigma_spec to ~0 and the GP zig-zags
            # through every individual spec point.
            "log_sigma_spec": (np.log(0.005), 0.0),
            "log_metric_t": (-6.0, 1.0) if self.additive_t else (-8.0, 6.0),
            "log_metric_w": (-6.0, 1.0) if self.additive_w else (-8.0, 6.0),
