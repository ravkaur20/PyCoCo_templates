...
# Kept in sync with GP2dim_utils_newlog (avoid importing that module's heavy stack from rimangle only).


def _mangled_wls_max_is_linear_angstrom(wls_arr):
	"""True if wls look like linear Å (same as GP2dim_utils_newlog)."""
...
def _mangled_flux_linear_from_log10(flux_arr):
	"""log10 F → linear F (same as GP2dim_utils_newlog)."""
