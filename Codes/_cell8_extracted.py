...
class FullMangledSeries_Class():
    ...
    def _wls_are_linear_angstrom(wls_arr):
        """Delegates to ``GP2dim_utils_newlog`` (imported as ``GP2dim`` in notebook 6)."""
    ...
    def __init__(self, snname, t0_fix, type_=None, spec_file=None, 
        ...
        # Log-spaced early extrapolation: dex span in log10(phase days) for np.linspace from gmin
        # (replaces a single 0.1-dex sliver; widens the grid in (−3,−1) before gap_fill).
        ...
        # Smaller value = more LC at late phase (e.g. log10>1, >10d); larger = sparser; tune for GP.
        ...
        # fill_gaps_phase_logspace: sub-threshold *linear* gap but large log span (default in GP2dim)
    ...
    def _extrap_phot_cadence_min_sep_dex(self, log_t):
        ...
        l_ = float(getattr(self, "extrap_phot_cadence_dex_late", 0.10))
...
# --- CHANGED: Look for the log flux columns ---
    ...
    def create_extended_spec_folder(self):
        save_plot_path = OUTPUT_DIR+'/%s/TwoDextended_spectra'%self.snname
    ...
    def extend_grid_all_spectraltimeseries(self):
        ...
        if self.mode=='extrapolate_spectra':
            ...
            if getattr(self, 'debug_training_grid', False) or self.verbose:
                ...
                # Training-grid diagnosis: if these are ~0, time columns are missing in that log range.
                for label, arr in [
