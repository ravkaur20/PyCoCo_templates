"""Collaborator GP (rjf): drop-in prediction matching classic ``run_2DGP_GRID`` grid stacking.

Supports **min–max** ``grid_norm_info`` (``GP2dim_utils_newlog``) and **z-score** coords
(``GP2dim_utils_newlog_zscore``), detected via ``coord_parametrization`` / keys.
...
import GP2dim_utils_newlog as _g_base

try:
	import GP2dim_utils_newlog_zscore as _gz
except ImportError:  # pragma: no cover
	_gz = None

_augment_t0_anchor_minmax = _g_base.augment_2dgp_training_t0_anchor
_apply_training_yerr_floors = _g_base._apply_training_yerr_floors  # noqa: SLF001
gp_dense_matrix_bytes_order_of_magnitude = _g_base.gp_dense_matrix_bytes_order_of_magnitude
log_prediction_phase_coverage = _g_base.log_prediction_phase_coverage


def _grid_norm_uses_zscore(gn: dict) -> bool:
	"""True when NB6 used ``GP2dim_utils_newlog_zscore.transform2LOG_reshape``."""
...
def maybe_plot_rjf_mu_raw_vs_post(
	...
	flag = bool(getattr(_pconf, "GP_RJF_PLOT_RAW_AND_PROCESSED", False)) or bool(
		getattr(GP2DIM_Class, "gp_rjf_plot_mu_raw_diagnostic", False)
	)
	if not flag:
		return None
	...
	from GP2dim_utils_newlog import scaled_ln_to_linear

	gn = GP2DIM_Class.grid_norm_info
	off = float(gn["offset"])
	scale = float(gn["scale_factor"])

	out_dir = os.path.join(GP2DIM_Class.save_plot_path, "diagnostics", "rjf_mu_raw_vs_post")
	os.makedirs(out_dir, exist_ok=True)

	if _grid_norm_uses_zscore(gn):
		if _gz is None:
			raise ImportError(
				"gp_rjf diagnostic plot needs GP2dim_utils_newlog_zscore for z-score grid_norm_info."
			)
		x1a = np.asarray(x1_fill, dtype=float)
		x2a = np.asarray(x2_fill, dtype=float)
		logwl = _gz.log10_wavelength_from_x1_norm(x1a, gn)
		wl_lin = np.power(10.0, np.clip(logwl, -50.0, 8.0))
		phase_lin = _gz.phase_days_from_norm_x2(x2a, gn)
	else:
	...
	fig.savefig(os.path.join(out_dir, "gp_rjf_mu_raw_vs_post_linflux.pdf"), bbox_inches="tight")
	plt.close(fig)
	return out_dir


def run_2DGP_GRID_rjf(
	GP2DIM_Class,
	y_data_nonan,
	y_data_nonan_err,
	x1_data_norm,
	...
	if run_gp_from_bundle is None:
		raise ImportError("gp_collab_rjf missing; vendor under Codes/gp_collab_rjf/.")
	...
	if bool(getattr(GP2DIM_Class, "gp_2d_anchor_t0", False)):
		...
		if use_z:
			x1_data_norm, x2_data_norm, y_data_nonan, y_data_nonan_err = _gz.augment_2dgp_training_t0_anchor(
				x1_data_norm,
				x2_data_norm,
				y_data_nonan,
				y_data_nonan_err,
		...
		if _log_progress:
			_gp_log("[run_2DGP_GRID_rjf] t0-anchor: +%i pseudo points" % _nu_before)
	...
	n_train = int(y.size)
	_gp_log(
		"[run_2DGP_GRID_rjf] start prior=%r N_train=%i X.shape=%s"
	...
	if getattr(GP2DIM_Class, "verbose", False) or getattr(GP2DIM_Class, "gp_print_training_size", True):
		...
		_gp_log("[run_2DGP_GRID_rjf] ~%.2f GB dense hint / Cholesky O(N³)" % (n2b / (1024.0**3),))
	...
	if span_wl <= 0.0:
		raise ValueError("run_2DGP_GRID_rjf: invalid log10 wavelength span.")
	...
	if extrap_mjds_arr.size == 0:
		raise ValueError("run_2DGP_GRID_rjf: extrap_mjds empty.")
	if bool(getattr(GP2DIM_Class, "gp_predict_dense_log_phase", False)):
		...
		_phase_before = extrap_mjds_arr.copy()
		extrap_mjds_arr = merge_extrap_mjds_dense_log_phase(extrap_mjds_arr, _dense_n)
		if _log_progress:
			_gp_log(
				"[run_2DGP_GRID_rjf] dense log-phase: %i → %i columns"
				% (len(_phase_before), len(extrap_mjds_arr))
			)
			log_prediction_phase_coverage(_phase_before, label="phase (before merge)")
			log_prediction_phase_coverage(extrap_mjds_arr, label="phase (after merge)")
	...
	if prior:
		if not (isinstance(points, np.ndarray) and isinstance(values, np.ndarray)):
			raise ValueError("run_2DGP_GRID_rjf: prior=True requires ndarray points/values.")
	...
	_cache_rel = getattr(_pconf, "GP_RJF_PRIOR_CACHE_SUBDIR", "diagnostics/rjf_prior_cache")
	prior_cache = os.path.join(GP2DIM_Class.save_plot_path, _cache_rel.replace("/", os.sep))
	os.makedirs(prior_cache, exist_ok=True)

	run_json = os.path.join(GP2DIM_Class.save_plot_path, "diagnostics", "rjf_gp_config.json")
	...
	user_kw = getattr(_pconf, "GP_RJF_KWARGS", {}) or {}
	if not isinstance(user_kw, dict):
		user_kw = {}
	user_kw = dict(user_kw)
	if not prior:
		user_kw["mean"] = "none"
	# Avoid duplicate keyword if ``GP_RJF_KWARGS`` sets ``predict_train`` (see ``pipeline_config``).
	predict_train_flag = bool(user_kw.pop("predict_train", False))

	y_compute = np.sqrt(yerr ** 2 + 1.0e-6 ** 2)

	_gp_log("[run_2DGP_GRID_rjf] collaborator fit + predict …")
	t0_inf = time.perf_counter()
	# ``early_time_cutoff`` in ``GP_RJF_KWARGS`` applies on ``X_fill[:,1]`` (same units as training).
	...
	if use_z and _gz is not None:
		...
		logwl_max = float(np.max(_gz.log10_wavelength_from_x1_norm(x1_fill, gn)))
	...
	_gp_log(
		"[run_2DGP_GRID_rjf] grid log10(λ): %s .. %s | log10(phase d): %s .. %s | N_pred=%i | coords=%s"
		% (logwl_min, logwl_max, lp_min, lp_max, mu_fill.size, "zscore" if use_z else "minmax")
	)

	maybe_plot_rjf_mu_raw_vs_post(GP2DIM_Class, x1_fill, x2_fill, mu_fill_raw, mu_fill, std_fill)
