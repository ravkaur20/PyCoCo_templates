"""2D ln-flux GP on log10(wavelength) x log10(phase) with **z-scored** training coordinates.

Fork of ``GP2dim_utils_newlog``: same flux / yerr handling as that file, but GP inputs use
``(x - mean_train) / std_train`` per axis instead of min–max scaling. Select via
``pipeline_config.USE_TWO_D_GP_ZSCORE_COORDS`` and notebook import of this module.

When **both** ``USE_TWO_D_GP_LINEAR_FLUX`` and ``USE_TWO_D_GP_ZSCORE_COORDS`` are True, notebook
imports take the **linear-flux** module (z-score applies only to the ln-flux path here).
"""
import numpy as np
import os
import pandas as pd
...
def x2_mask_for_phase(x2_fill, phase_log, gn):
	"""Rows of the prediction grid matching ``phase_log`` (log10 days). ``gn`` is ``grid_norm_info`` (z-score)."""
...
def phase_days_from_norm_x2(x2_norm, gn):
	"""Linear phase (days) from normalized x2; ``log10(days) = x2_mean + x2_std * x2_norm``."""
	lp = np.asarray(gn["x2_mean"], dtype=float) + np.asarray(gn["x2_std"], dtype=float) * np.asarray(
		x2_norm, dtype=float
	)
...
def log10_wavelength_from_x1_norm(x1_norm, gn):
	"""log10(wavelength) from normalized x1 (z-score axis)."""
	return np.asarray(gn["x1_mean"], dtype=float) + np.asarray(gn["x1_std"], dtype=float) * np.asarray(
		x1_norm, dtype=float
	)
...
def transform2LOG_reshape(GP2DIM_Class, raw_numbers, raw_numbers_err,  off_xa, off_ya):
	...
	``grid_norm_info``); flux scaling is unchanged from ``GP2dim_utils_newlog``.
	"""
	LN10 = np.log(10.0)
...
def make_plots(GP2DIM_Class, y_data_nonan, y_data_nonan_err, x1_data_norm, x2_data_norm):	
	...
	x_phase = np.asarray(gn["x2_mean"], dtype=float) + np.asarray(gn["x2_std"], dtype=float) * np.asarray(
		x2_data_norm, dtype=float
	)
	y_logwl = np.asarray(gn["x1_mean"], dtype=float) + np.asarray(gn["x1_std"], dtype=float) * np.asarray(
		x1_data_norm, dtype=float
	)
...
def augment_2dgp_training_t0_anchor(
	...
	*,
	...
	x2_mean = float(grid_norm_info["x2_mean"])
	x2_std = float(grid_norm_info["x2_std"])
	ln_flux = float(log10_flux_cap) * LN10
	y_cap = (ln_flux - offset) / sf
	sigma = float(log10_flux_err) * LN10 / sf
	x2a = (float(log_phase_anchor) - x2_mean) / x2_std
...
def run_2DGP_GRID(GP2DIM_Class, y_data_nonan, y_data_nonan_err, x1_data_norm, x2_data_norm,\
	...
	_gp_log(
		...
		% (
			float(np.min(float(gn["x2_mean"]) + float(gn["x2_std"]) * x2_fill)),
			float(np.max(float(gn["x2_mean"]) + float(gn["x2_std"]) * x2_fill)),
		)
	)
	...
	maybe_save_gp_minimal_export(
		...
		gp_module="GP2dim_utils_newlog_zscore",
...
def transform_back_andPlot(GP2DIM_Class, x1_fill, x2_fill, mu_fill, std_fill, y_data_nonan):
	...
	x1m, x1s = float(gn["x1_mean"]), float(gn["x1_std"])
	x2m, x2s = float(gn["x2_mean"]), float(gn["x2_std"])
	x2_train_min = float(gn["x2_train_min"])
