...
RY_GP = os.path.join(CODES_DIR, "ryan_gp")
...
class TestRyanStripBands(unittest.TestCase):
	def test_strip_removes_matching_photometry_only(self):
		strip_py = os.path.join(RY_GP, "strip_photometry_bands.py")
		...
		with tempfile.TemporaryDirectory() as td:
			...
			subprocess.run(cmd, cwd=RY_GP, check=True)
