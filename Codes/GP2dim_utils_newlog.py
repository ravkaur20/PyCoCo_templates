...
def x2_mask_for_phase(x2_fill, phase_log, offset2, norm2):
	"""Rows of the prediction grid matching spectrophotometry column ``phase_log`` (log10 days since explosion)."""
	target = (float(phase_log) - float(offset2)) / float(norm2)
...
def phase_days_from_norm_x2(x2_norm, offset2, norm2):
	"""Phase in days from GP column ``x2_norm`` with ``log10(days) = offset2 + norm2 * x2_norm`` (``make_plots`` / training convention)."""
	lp = np.asarray(offset2, dtype=float) + np.asarray(norm2, dtype=float) * np.asarray(x2_norm, dtype=float)
...
def make_plots(GP2DIM_Class, y_data_nonan, y_data_nonan_err, x1_data_norm, x2_data_norm):	
	...
	offset2, norm1, norm2 = gn['offset2'], gn['norm1'], gn['norm2']
	# Absolute log10(phase days); norm2*x2_norm alone is (phase - offset2)
	x_phase = offset2 + norm2 * x2_data_norm
	y_logwl = norm1 * x1_data_norm
