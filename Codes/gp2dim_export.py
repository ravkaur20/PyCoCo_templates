...
def _ryan_gp_dir() -> str:
	return os.path.join(os.path.dirname(os.path.abspath(__file__)), "ryan_gp")


def _import_spec_bundle_helpers():
	"""Import from ``Codes/ryan_gp`` (not on sys.path by default)."""
	_ry = _ryan_gp_dir()
...
def save_gp_minimal_bundle(
	...
	*,
	...
	Optional **collaborator layout** keys (for ``ryan_gp/iterate_gp_surface_bundle_scale``):
	...
	if sbid is not None:
		meta["spec_bundle_id_export"] = (
			"int32 per row: -1 photometry; non-negative spectroscopic bundle (see ryan_gp/spec_bundle_id_assign)"
		)
