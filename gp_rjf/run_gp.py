...
def _make_neg_ll(
    ...
    eval_count = {"n": 0, "best": np.inf, "best_theta": None}
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("--input", default=DEFAULT_BUNDLE)
    p.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR)
    p.add_argument("--tag", default=None)

    p.add_argument("--kernel-time", choices=gu.KERNEL_NAMES, default="matern52")
    p.add_argument("--kernel-wls", choices=("matern32", "matern52", "expsq"), default="matern52")
    ...
    p.add_argument("--lw-short", type=float, default=0.005, help="warm-start SHORT metric on wavelength axis when --additive-wls; default 0.005 (length~0.07) matches kilonova spectral features ~thousands of km/s")
    ...
    p.add_argument("--sigma-spec", type=float, default=0.005)
    p.add_argument("--enforce-mono-early", dest="enforce_mono_early", action="store_true", default=True,
                   help="post-process mu so flux is monotonically increasing in time for normalized log10(phase) < --early-time-cutoff (C^1-smooth extrapolation)")
    ...
    p.add_argument("--enforce-blue-early", dest="enforce_blue_early", action="store_true", default=True,
                   help="for phase < --early-time-cutoff, enforce mu non-increasing in wls (spectrum increasing toward shorter wavelengths, hot-blackbody-like)")
    ...
    p.add_argument(
        ...
        default=2500,
        help=(
            "Use a random subsample of size N during optimization (Cholesky is "
            "O(N^3); subsampling 2.5k saves >40x). 0 disables subsampling. "
            "All phot points are always retained."
        ),
    )
    ...
    point_class = gu.classify_points(X, threshold=args.phot_spec_threshold)
